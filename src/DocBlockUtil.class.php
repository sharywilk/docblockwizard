 ...
 * Class DocBlockUtil
 *
 * Class for modifying docblocks in PHP files; appending,
 * changing and removing annotations
 *
 */
...
abstract class WikiaBaseTest extends PHPUnit_Framework_TestCase
{
    ...
    protected function removeSlowTestAnnotation()
    {
        ...
        $className = get_class($this);
        $methodName = $this->getName(false);
        $regexSlowGroup = '/^\s*\*\s*@group\s+Slow\s*\n/m';
        $regexSlowExecTime = '/^\s*\*\s*@slowExecutionTime\s+([0-9\.]+\s*(ms?)\s*\n)/m';
